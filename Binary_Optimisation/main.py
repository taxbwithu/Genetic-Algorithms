...
class GeneticAlgorithm(tk.Frame):
    ...
    def algorithm(self):
        ...
        new_population = [[],[]]
        t_start = time.time()
        chain_sections = calc_sections(int(self.entry_acc.get()))
        populations = generate_populations(chain_sections, int(self.entry_pop.get()))
        function_values = function(populations, chain_sections)
        ...
        for ep in range(epochs):
            ...
            if self.variable.get() == "minimisation":
                elite_f, elite_p = sort(function_values, populations, False)
            else:
                elite_f, elite_p = sort(function_values, populations, True)
            for i in range(int(self.entry_elite.get())):
                new_population[0].append(elite_p[0][i])
                new_population[1].append(elite_p[1][i])
            ...
            crossed_population = crossover(populations, selected_populations, new_population, self.entry_cross.get(),
                                           self.variable2.get())
            mutated_population = mutation(crossed_population, self.entry_mutation.get(), self.variable3.get())
            inverted_population = inversion(mutated_population, self.entry_inversion.get())
            populations = [[], []]
            for i in range(len(inverted_population[0])):
                elite = 0
                if i < len(elitex1):
                    populations[0].append(elitex1[i])
                    populations[1].append(elitex2[i])
                    elite = elite + 1
                else:
                    populations[0].append(inverted_population[0][i - elite])
                    populations[1].append(inverted_population[1][i - elite])
            function_values = function(populations, chain_sections)
            if self.variable.get() == "minimisation":
        ...
        bestx1 = bin_to_dec(final_p[0][0], chain_sections)
        bestx2 = bin_to_dec(final_p[1][0], chain_sections)
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.variable2.set("one_point")
        self.variable3 = tk.StringVar(root)
        self.variable3.set("marginal")
        ...
        label_accuracy = tk.Label(root, text="Dokładność Chromosomu")
        ...
        label_inversion = tk.Label(root, text="Prawdopodobieństwo inwersji")
        ...
        self.entry_acc = tk.Entry(root)
        self.entry_acc.insert(tk.END, '6')
        ...
        self.entry_cross_type = tk.OptionMenu(root, self.variable2, "one_point", "two_point", "three_point")
        self.entry_mutation = tk.Entry(root)
        self.entry_mutation.insert(tk.END, '0.2')
        self.entry_mutation_type = tk.OptionMenu(root, self.variable3, "marginal", "one_point", "two_point")
        self.entry_inversion = tk.Entry(root)
        self.entry_inversion.insert(tk.END, '0.01')
        ...
        label_accuracy.grid(row=0, column=0)
        ...
        label_inversion.grid(row=12, column=0)
        ...
        self.entry_acc.grid(row=1, column=0)
        ...
        self.entry_inversion.grid(row=13, column=0)
